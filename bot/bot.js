...
class ShopBot {
  ...
  async handleCallbackQuery(query) {
    ...
    if (data === 'menu_main') {
      ...
      await this.bot.editMessageText(`👋 Halo *${name}*! Pilih menu:`, {
    ...
    // ── Buat order ──
      ...
      // ── Pakasir: generate link pembayaran otomatis ──
      if (this.pakasir) {
        try {
          const payUrl = this.pakasir.getPaymentUrl(orderId, p.price, { qrisOnly: false });
          await this.bot.editMessageText(
            `✅ *Pesanan Dibuat!*\n\n🔖 Order ID: \`${orderId}\`\n📦 *${p.name}*\n💰 *${formatCurrency(p.price)}*\n\n💳 *Bayar sekarang via link di bawah:*`,
            { chat_id: chatId, message_id: msgId, parse_mode: 'Markdown',
              reply_markup: { inline_keyboard: [
                [{ text: '💳 Bayar Sekarang', url: payUrl }],
                [{ text: '📋 Cek Status Pembayaran', callback_data: `cek_pay_${orderId}` }],
          ...
          // Fallback ke manual jika Pakasir error
          await this.bot.editMessageText(
            `✅ *Pesanan Dibuat!*\n\n🔖 \`${orderId}\`\n📦 *${p.name}*\n💰 *${formatCurrency(p.price)}*`,
            { chat_id: chatId, message_id: msgId, parse_mode: 'Markdown',
              reply_markup: { inline_keyboard: [
        ...
        // Tanpa Pakasir — manual seperti sebelumnya
        await this.bot.editMessageText(
          `✅ *Pesanan Dibuat!*\n\n🔖 Order ID: \`${orderId}\`\n📦 Produk: *${p.name}*\n💰 Harga: *${formatCurrency(p.price)}*\n📊 Status: Menunggu Pembayaran`,
          { chat_id: chatId, message_id: msgId, parse_mode: 'Markdown',
            reply_markup: { inline_keyboard: [
              [{ text: '💳 Lihat Cara Pembayaran', callback_data: `show_pay_${orderId}` }],
              [{ text: '❌ Batalkan Order', callback_data: `cancel_order_${orderId}` }],
            ]}
          }
        );
      ...
      for (const adminId of this.adminIds) {
        await this.bot.sendMessage(adminId,
          `🆕 *Pesanan Baru!*\n👤 @${query.from.username||query.from.first_name} (${userId})\n🔖 \`${orderId}\`\n📦 ${p.name}\n💰 ${formatCurrency(p.price)}\n💳 ${this.pakasir ? 'Auto Payment (Pakasir)' : 'Manual'}`,
          { parse_mode: 'Markdown' }
        );
      }
    }
  ...
  async handleDynamicTriggers(msg) {
    ...
    // Cek state user (misal: waiting_order_id)
