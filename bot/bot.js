...
class ShopBot {
  ...
  async sendMainMenu(chatId, name) {
    ...
    const stockSummary = this.buildStockSummary();
  ...
  // ── Bangun teks ringkasan stok semua produk ──────────────────
  buildStockSummary() {
    const products = this.db.getProducts();
    if (!products.length) return '📭 _Belum ada produk tersedia saat ini._';

    let text = '📦 *Stok Tersedia Saat Ini:*\n';
    for (const p of products) {
      const stock = this.db.getStockCount(p.id);
      const statusIcon = p.preorder_only ? '📋' : stock > 0 ? '✅' : '❌';
      const stockLabel = p.preorder_only ? 'Pre-Order' : stock > 0 ? `${stock} stok` : 'Habis';
      text += `${statusIcon} ${p.name} — ${formatCurrency(p.price)} _(${stockLabel})_\n`;
    }
    return text;
  }
